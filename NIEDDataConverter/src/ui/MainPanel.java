...
public class MainPanel extends JPanel implements ActionListener{
	File file;
	File outputFile;
	JFileChooser filechooser,filechooser2;
	JButton[] buttons;
	BufferedWriter bw;
	...
	public MainPanel(){
		...
		add(label);add(buttons[0]);add(buttons[1]);
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource() == buttons[1]){
			...
			int selected = filechooser2.showSaveDialog(this);
		    if (selected == JFileChooser.APPROVE_OPTION){
		    	try {
					bw = new BufferedWriter(new FileWriter( filechooser2.getSelectedFile() + ".txt"));
					for(int i = 0;i<data.size();i++){
						try {
